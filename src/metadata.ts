 ...
 * Edited by : Francesco
 * Edited at : 2025-12-09 20:59
 *
 * Copyright (c) 2025 Xevolab S.R.L.
 */

import { stripPemHeaderAndFooter, generateUniqueId as generateUniqueIdDefault } from "./crypto";
import {
...
export const generateServiceProviderMetadata = (
	...
	if (identifierFormat != null) metadata.EntityDescriptor.SPSSODescriptor.NameIDFormat = identifierFormat;

	if (wantAssertionsSigned) metadata.EntityDescriptor.SPSSODescriptor["@WantAssertionsSigned"] = true;
	...
	if (attributeConsumingServices) {
		metadata.EntityDescriptor.SPSSODescriptor.AttributeConsumingService = attributeConsumingServices.map(
			(service, i) => ({
				"@index": i.toString(),
				...service,
			} as XMLObject),
