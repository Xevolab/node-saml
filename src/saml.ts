/*
 * Author    : Francesco
 * Created at: 2025-12-09 20:24
 * Edited by : Francesco
 * Edited at : 2025-12-09 20:47
 *
...
const resolveAndParseKeyInfosToPem = async ({
	...
	if (Array.isArray(certs)) return certs.map((cert, index) => keyInfoToPem(cert, "CERTIFICATE", `idpCert[${index}]`));
...
class SAML {
	...
	initialize(ctorOptions: SamlConfig): SamlOptions {
		if (!ctorOptions) throw new TypeError("SamlOptions required on construction");
		...
		if (!Object.values(ValidateInResponseTo).includes(options.validateInResponseTo)) throw new TypeError("validateInResponseTo must be one of ['never', 'ifPresent', 'always']");
		 ...
		 */
		if (!["exact", "minimum", "maximum", "better"].includes(options.racComparison)) throw new TypeError("racComparison must be one of ['exact', 'minimum', 'maximum', 'better']");
	...
	protected async generateAuthorizeRequestAsync(
		...
		if (this.mustValidateInResponseTo(true)) await this.cacheProvider.saveAsync(id, instant);
		...
		const request: AuthorizeRequestXML = {
			"samlp:AuthnRequest": {
				...
				"saml:Issuer": {
					...
					"@NameQualifier": this.options.callbackUrl,
		...
		if (samlAuthnRequestExtensions != null) {
			if (typeof samlAuthnRequestExtensions !== "object") throw new TypeError("samlAuthnRequestExtensions should be Object");
		...
		const nameIDPolicy: XMLInput = {
			"@xmlns:samlp": "urn:oasis:names:tc:SAML:2.0:protocol",
			"@AllowCreate": this.options.allowCreate ? true : undefined,
		};

		if (this.options.identifierFormat != null) nameIDPolicy["@Format"] = this.options.identifierFormat;


		if (this.options.spNameQualifier != null) nameIDPolicy["@SPNameQualifier"] = this.options.spNameQualifier;
		...
		if (this.options.attributeConsumingServiceIndex != null) request["samlp:AuthnRequest"]["@AttributeConsumingServiceIndex"] = this.options.attributeConsumingServiceIndex;


		if (this.options.providerName != null) request["samlp:AuthnRequest"]["@ProviderName"] = this.options.providerName;


		if (this.options.scoping != null) {
			const scoping: XMLInput = { "@xmlns:samlp": "urn:oasis:names:tc:SAML:2.0:protocol" };

			if (typeof this.options.scoping.proxyCount === "number") scoping["@ProxyCount"] = this.options.scoping.proxyCount;
			...
			if (this.options.scoping.idpList) {
				scoping["samlp:IDPList"] = this.options.scoping.idpList.map(
					(idpListItem: SamlIDPListConfig) => {
						...
						if (idpListItem.entries) {
							formattedIdpListItem["samlp:IDPEntry"] = idpListItem.entries.map(
								(entry: SamlIDPEntryConfig) => {
									...
									if (entry.name) formattedEntry["@Name"] = entry.name;


									if (entry.loc) formattedEntry["@Loc"] = entry.loc;
						...
						if (idpListItem.getComplete) formattedIdpListItem["samlp:GetComplete"] = idpListItem.getComplete;
			...
			if (this.options.scoping.requesterId) scoping["samlp:RequesterID"] = this.options.scoping.requesterId;
		...
		if (isHttpPostBinding && isValidSamlSigningOptions(this.options)) stringRequest = signAuthnRequestPost(stringRequest, this.options);
	...
	async _generateLogoutRequest(this: SAML, user: Profile): Promise<string> {
		...
		const { samlLogoutRequestExtensions } = this.options;
		if (samlLogoutRequestExtensions != null) {
			if (typeof samlLogoutRequestExtensions !== "object") throw new TypeError("samlLogoutRequestExtensions should be Object");
		...
		if (user.nameQualifier != null) request["samlp:LogoutRequest"]["saml:NameID"]["@NameQualifier"] = user.nameQualifier;


		if (user.spNameQualifier != null) request["samlp:LogoutRequest"]["saml:NameID"]["@SPNameQualifier"] = user.spNameQualifier;
	...
	async _requestToUrlAsync(
		...
		let buffer: Buffer;
		if (this.options.skipRequestCompression) buffer = Buffer.from(requestOrResponse, "utf8");
		else buffer = await deflateRawAsync(requestOrResponse);
		...
		if (operation === "logout") {
			if (this.options.logoutUrl) target = new URL(this.options.logoutUrl);
		}
		...
		if (isValidSamlSigningOptions(this.options)) {
			if (!this.options.entryPoint) throw new Error('"entryPoint" config parameter is required for signed messages');
	...
	_getAdditionalParams(
		...
		if (typeof relayState === "string" && relayState.length > 0) additionalParams.RelayState = relayState;
	...
	async getAuthorizeUrlAsync(
		...
		return {
			...
			url: await this._requestToUrlAsync(
				request,
				null,
				operation,
	...
	async getAuthorizeMessageAsync(
		...
		host?: string,
		options?: AuthOptions,
	): Promise<{ AuthnRequest: string, request: querystring.ParsedUrlQueryInput, url: string }> {
		assertRequired(this.options.entryPoint, "entryPoint is required");

		const request = await this.generateAuthorizeRequestAsync(this.options.passive, true);
		let buffer: Buffer;
		if (this.options.skipRequestCompression) buffer = Buffer.from(request, "utf8");
		else buffer = await deflateRawAsync(request);
		...
		return {
			AuthnRequest: request,
			request: samlMessage.SAMLRequest as unknown as querystring.ParsedUrlQueryInput,
			url: await this._requestToUrlAsync(
				request,
				null,
				operation,
	...
	async getAuthorizeFormAsync(
		...
		const { request: samlMessage } = await this.getAuthorizeMessageAsync(RelayState, host, options);

		const formInputs = Object.keys(samlMessage)
			.map(k => `<input type="hidden" name="${k}" value="${quoteattr(samlMessage[k])}" />`)
			.join("\r\n");
	...
	protected async getSignedAssertion(signedXml: string): Promise<string | null> {
		...
		if (rootNode.localName === "Response") {
			...
			if (assertions.length == 1) return assertions[0].toString();
			...
			if (encryptedAssertions.length === 1) {
				...
				if (decryptedAssertion.localName !== "Assertion") throw new Error("Invalid EncryptedAssertion content");
	...
	async validatePostResponseAsync(
		...
		try {
			...
			if (responseVerifiedXml) validSignature = true;


			if (this.options.wantAuthnResponseSigned === true && validSignature === false) throw new Error("Invalid document signature");
			...
			if (assertions.length == 1) {
				if (this.options.wantAssertionsSigned || !validSignature) {
					...
					if (!assertionVerifiedXml) throw new Error("Invalid signature");
				}
			}
			...
			if (encryptedAssertions.length == 1) {
				...
				if (this.options.wantAssertionsSigned || !validSignature) {
					...
					if (decryptedAssertionVerifiedXml == null) throw new Error("Invalid signature from encrypted assertion");
				}
			}
			...
			if (verifiedXml && assertions.length + encryptedAssertions.length == 1) {
				...
				if (signedAssertion == null) throw new Error("Cannot obtain assertion from signed data");
			...
			if (response) {
					...
					if (status) {
						...
						if (
							...
							if (
								...
								if (!validSignature) throw new Error("Invalid signature: NoPassive");
				...
				if (!validSignature) throw new Error("Invalid signature: No response found");

				const logoutResponse = xmljsDoc.LogoutResponse;
				if (logoutResponse) return { profile: null, loggedOut: true };
			...
			if (this.mustValidateInResponseTo(Boolean(inResponseTo))) await this.cacheProvider.removeAsync(inResponseTo);
	...
	protected async hasValidSignatureForRedirect(
		...
		if (container.Signature) {
			...
			if (getParam("RelayState")) urlString += `&${getParam("RelayState")}`;


			urlString += `&${getParam("SigAlg")}`;

			const pemFiles = await this.getKeyInfosAsPem();
			const hasValidQuerySignature = pemFiles.some(pemFile => this.validateSignatureForRedirect(
				urlString,
				container.Signature as string,
				container.SigAlg as string,
				pemFile,
			));
			if (!hasValidQuerySignature) throw new Error("Invalid query signature");
		}
		else {
			return true;
		}
	...
	protected validateSignatureForRedirect(
		...
		let matchingAlgo;
		if (i > -1) matchingAlgo = crypto.getHashes()[i];
		else throw new Error(`${alg} is not supported`);
	...
	protected async verifyLogoutResponse(doc: XMLOutput): Promise<void> {
		...
		if (inResponseTo) return this.validateInResponseTo(inResponseTo);
	}
		...
		if (this.options.idpIssuer != null) {
			...
			if (issuer) {
				if (issuer[0]._ !== this.options.idpIssuer) {
					throw new Error(
						`Unknown SAML issuer. Expected: ${this.options.idpIssuer
						} Received: ${issuer[0]._}`,
					);
				}
			}
	...
	protected async processValidlySignedAssertionAsync(
		...
		getInResponseTo: {
			...
			if (issuer && issuer[0]._) profile.issuer = issuer[0]._;


			if (inResponseTo != null) profile.inResponseTo = inResponseTo;


			const authnStatement = assertion.AuthnStatement;
			if (authnStatement) if (authnStatement[0].$ && authnStatement[0].$.SessionIndex) profile.sessionIndex = authnStatement[0].$.SessionIndex;
			...
			if (subject) {
				...
				if (subjectConfirmation != null) confirmData = subjectConfirmation.SubjectConfirmationData[0];
			}
					...
					if (confirmData?.$) {
						...
						if (inResponseTo && subjectInResponseTo && subjectInResponseTo != inResponseTo) {
							...
							if (result) {
								...
								if (nowMs < createdAt.getTime() + this.options.requestIdExpirationPeriodMs) foundValidInResponseTo = true;
							}
							await this.cacheProvider.removeAsync(inResponseTo);
							if (!foundValidInResponseTo) throw new Error("SubjectInResponseTo is not valid");

							break getInResponseTo;
						}
					}
				}
				else if (subjectConfirmations != null && subjectConfirmation == null) {
		...
		if (attributeStatement) {
			...
			const attrValueMapper = (value: XMLObject) => {
				const hasChildren = Object.keys(value).some(cur => cur !== "_" && cur !== "$");
			...
			if (attributes.length > 0) {
				...
				attributes.forEach(attribute => {
					...
					const value: XMLValue | XMLValue[] = attribute.AttributeValue.length === 1
		...
		if (!profile.email && profile.mail) profile.email = profile.mail;
	...
	protected checkTimestampsValidityError(
		...
		if (notOnOrAfter) {
			...
			if (nowMs - this.options.acceptedClockSkewMs >= notOnOrAfterMs) return new Error("SAML assertion expired: clocks skewed too much");
		}
		if (maxTimeLimitMs) if (nowMs - this.options.acceptedClockSkewMs >= maxTimeLimitMs) return new Error("SAML assertion expired: assertion too old");
	...
	protected checkAudienceValidityError(
		...
		if (!audienceRestrictions || audienceRestrictions.length < 1) return new Error("SAML assertion has no AudienceRestriction");

		const errors = audienceRestrictions
			.map(restriction => {
				if (!restriction.Audience || !restriction.Audience[0] || !restriction.Audience[0]._) return new Error("SAML assertion AudienceRestriction has no Audience value");
				...
				if (restriction.Audience.every(audience => audience._ !== expectedAudience)) {
					return new Error(
						...
						} Received: ${restriction.Audience.map(audience => audience._).join(", ")}`,
					);
				}
				return null;
			})
	...
	async validatePostRequestAsync(
		...
		const pemFiles = await this.getKeyInfosAsPem();
		if (!_validateSignature(xml, dom.documentElement, pemFiles)) throw new Error("Invalid signature on documentElement");
	...
	protected async processValidlySignedPostRequestAsync(
		...
		if (request) {
			const profile = {} as Profile;
			if (request.$.ID) profile.ID = request.$.ID;
			else throw new Error("Missing SAML LogoutRequest ID");

			const issuer = request.Issuer;
			if (issuer && issuer[0]._) profile.issuer = issuer[0]._;
			else throw new Error("Missing SAML issuer");

			const nameID = await getNameIdAsync(dom, this.options.decryptionPvk ?? null);
			if (nameID.value) {
				profile.nameID = nameID.value;
				if (nameID.format) profile.nameIDFormat = nameID.format;
			}
			else {
				throw new Error("Missing SAML NameID");
			}
			const sessionIndex = request.SessionIndex;
			if (sessionIndex) profile.sessionIndex = sessionIndex[0]._;
	...
	protected async processValidlySignedSamlLogoutAsync(
		...
		if (response) return { profile: null, loggedOut: true };
		if (request) return await this.processValidlySignedPostRequestAsync(doc, dom);
	...
	protected calcMaxAgeAssertionTime(
		...
		if (maxAssertionAgeMs === 0) return notOnOrAfterMs;
